...
| **OAuth Callback Multi-Origin Redirect (#8 / F2)** | Known behavior: `oauthCallbackRedirectURL` builds an ABSOLUTE URL from `PUBLIC_BASE_URL` (`api/internal/handler/auth_handler.go:479`). A user reaching the stack via a non-canonical origin (e.g. localhost:3456 while `PUBLIC_BASE_URL=tuyens-mac-mini.tail48a1b0.ts.net`) crosses origin on the 302 → host-only `cgp_session` cookie is NOT sent → `GET /me` 401 → login appears broken. | Single-origin access is the documented contract; e2e runs REQUIRE `PUBLIC_BASE_URL` aligned with the actual access URL (tester 2026-09-24 j1 :24/:112 saga). | Follow-up candidate: honor request origin behind an explicit allowlist (`request.Host ∈ {PUBLIC_BASE_URL, localhost, LAN-IPs}`) so a single stack serves multiple access points without re-deploying env. Drift note: `e2e/helpers/auth.ts` originally APPENDED a custom `code=` param; Go's `c.Query("code")` is FIRST-WINS so the append was silently ignored, collapsing every mock login onto one shared identity (`dev-user@test.vn`). Fixed by `f70f21c` — helper now REPLACES (not appends) the `code` param. |
